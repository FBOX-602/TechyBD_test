...
export async function listAllItems() {
  ...
  const endpoint = `${url}/rest/v1/cms_items?select=*&order=sort_order.asc,created_at.asc`;

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 1200);

  try {
    const response = await fetch(endpoint, { headers: getSupabaseHeaders(), signal: controller.signal });
    ...
    if (Array.isArray(rows)) {
      rows.forEach((row) => {
        if (collections[row.resource]) {
          collections[row.resource].push(toPublicItem(row));
        }
      });
    }
    ...
    clearTimeout(timeoutId);
    console.warn("[CMS DB] Supabase listAllItems timed out or failed (1.2s limit):", err.message);
    return null;
  }
}
...
export async function listItems(resource) {
  ...
  const collections = await listAllItems();
  if (collections && Array.isArray(collections[resource])) {
...
export async function createItem(resource, payload) {
  ...
  // Ensure unique UUID for every single item created
